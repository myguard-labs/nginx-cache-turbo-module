...
# RETARGET (adoption step 32): normalise codespell's exit status into this
# suite's 0/1/2 contract instead of letting it escape.
#
# codespell exits 65 when it finds misspellings (its own convention), and
# `exec`/a bare final call would make 65 this checker's status. run-all.sh
# classifies anything that is not 0 or 2 as "findings", so 65 happens to land
# in the right bucket there -- but the hook, lint.yml and a human running this
# script directly all read the raw status, and 65 is not a documented value for
# any of them. Worse, a FUTURE codespell that used 2 for "findings" would be
# reported as "tool missing" and mask a real red. Map explicitly.
rc=0
