...
# RETARGETED for nginx-cache-turbo-module (adoption step 13/14). The skeleton
# ships three label sets; this repo selects on exactly one, the bare
# `[self-hosted, builder02]` form, which is also the only label declared in
# .github/actionlint.yaml. The narrower set is the point: an approved-selector
# list copied wider than the repo actually uses approves a selector nobody
# reviewed, and `lxc`/`docker` are runner families no `runs-on` here names.
# Verified against all 15 selectors in .github/workflows/ on 2026-08-10.
#
# POOL_OWNED=yes: origin is myguard-labs, and the builder02 pool answers these
# labels. A fork of this repo inherits a selector it cannot satisfy -- that is
# what the fork arm above is for, and why it is not a parameter.
TRUST_SPLITS = frozenset(
    _trust_split(labels)
    for labels in (["self-hosted", "builder02"],)
)
...
# RETARGET (adoption step 32), carried over from this repo's own
# ci/tools/lint-ci-ports.sh checks 3: two ways a line NAMES a binder without
# being one. Both were exemptions in the bash checker this file replaces, and
# dropping them in the port produced two findings that are false by inspection
# -- which is the failure mode a ported checker is supposed to avoid, since a
# gate red on arrival gets bypassed rather than read.
#
# 1. `python3 -m py_compile ci/tools/test_runtime.py` checks syntax. It starts
#    no server and binds no port, so the validation job owes no band.
# 2. A line mentioning the driver only inside a COMMENT is prose. _body()
#    already strips comments via the YAML round-trip for the mapping keys, but
#    a `run:` block's shell comments survive into the dumped scalar.
...
def _body(node: dict) -> str:
    ...
    # Drop shell comments and non-binding invocations before the substring
    # questions are asked (see NON_BINDING_RE). Prose that merely NAMES the
    # driver must not make a job runtime-bearing: this repo's validation job
    # discusses ci/tools/test_runtime.py in a comment and only ever runs
    # py_compile over it, and counting that as a bind demands a port band for
    # a syntax check. Stripping is the safe direction -- it can only ever make
    # this checker ask for less, never approve a real binder it cannot see,
    # because a real binder is not written inside a comment.
    # Split on the ESCAPED newlines too. safe_dump folds a multi-line `run:`
    # into one quoted scalar containing literal "\n", so a naive per-dumped-line
    # strip would cut at the first '#' and discard every real command after it
    # -- including the `--port $TEST_BASE_PORT` that proves a job is wired.
    # Verified on build-test.yml:prove, whose fencing step is exactly that
    # shape. Fail direction matters: losing real text here would make a
    # misconfigured job look clean.
    kept = []
...
def _order_finding(where: str, node: dict) -> str | None:
    ...
    if first_bind == verify:
        ...
        # future edit to BINDER_RE would turn it into an AttributeError
        # traceback inside a linter, i.e. exit 2 "could not run" for what is
        # really a clean step.
    ...
    for path in workflows():
        ...
        for job, node in jobs(doc):
            ...
            if PROVE_RE.search(body):
                ...
                # RETARGET (adoption step 32): TEST_NGINX_RANDOMIZE is the one
                # case where a runtime-bearing job legitimately does NOT bind
                # its declared band, and this repo's prove job uses it. It is
                # what makes `prove -jN` safe: Test::Nginx's Util.pm
                # gen_rand_port picks a port PER PARALLEL JOB and binds only
                # ports it has proved free, so pinning TEST_NGINX_PORT would
                # re-share the resource the randomization just separated and
                # reintroduce the collision the bands exist to prevent. The
                # band is still declared and still fenced (build-test.yml:prove
                # holds every sibling band while it runs), which is what
                # reserves this job's territory on the shared runner.
                #
                # Carried over from ci/tools/lint-ci-ports.sh, which had this
                # exemption; the skeleton's checker does not, because the
                # skeleton's prove job does not randomize.
