...
check_decl() {
	...
	if ! sed -n "/ngx_int_t ${fn}(/,/;/p" "$header" |
		grep -qF 'NGX_HTTP_CACHE_TURBO_MUST_CHECK'; then
...
set +e
grep -R -n -E --include='*.[ch]' \
	'\(void\)[[:space:]]*ngx_http_cache_turbo_(digest|digest_final|vary_prepare(_lazy)?|vary_apply|variant_hash|marker_hash|marker_store(_key)?|variant_index_name)\(' \
	"$REPO/src"
grep_rc=$?
set -e
...
if [ "$grep_rc" -ne 1 ]; then
	echo "✗ discarded-result scan failed (grep exit $grep_rc)" >&2
	exit 1
fi
