...
_ARG_NEEDLE_TABLE = {
    ...
    "mediawiki":   ("/ct-mediawiki-args/", ["veaction", "returnto",
                                                 ...
                                                 "action=delete", "action=protect",
                                                 "action=unprotect", "action=purge",
                                                 "action=rollback", "action=revert",
                                                 "action=watch", "action=unwatch",
                                                 "action=markpatrolled", "action=mcrundo",
                                                 "action=mcrrestore"]),
    "ghost":       ("/ct-ghost-args/", ["uuid", "key", "token", "gift"]),
    "invision":    ("/ips/", ["do=compose", "do=post", "do=reply",
                               "do=report", "module=messaging"]),
    "smf":         ("/smf/", ["action=admin", "action=login", "action=login2",
                                 "action=logintfa", "action=logout", "action=profile",
                                 "action=pm", "action=post", "action=post2",
                                 "action=moderate", "action=reporttm", "action=xmlhttp"]),
    "yabb":        ("/ct-yabb-args/", ["action=post", "action=post2",
                                           "action=login", "action=login2",
                                           "action=logout", "action=register",
                                           "action=register2", "action=admin",
                                           "action=pm", "action=imsend",
                                           "action=imsend2"]),
    "mybb":        ("/mybb/", ["action=login", "action=do_login",
                                  "action=logout", "action=do_logout",
                                  "action=register", "action=do_register",
                                  "action=activate", "action=lostpw",
                                  "action=do_lostpw", "action=resetpassword"]),
    "spip":        ("/ct-spip/", ["action", "var_mode"]),
    "bugzilla":    ("/ct-bugzilla/", ["Bugzilla_api_key", "api_key",
                                         "Bugzilla_api_token", "Bugzilla_token", "Bugzilla_login",
                                         "Bugzilla_password", "Bugzilla_login_token",
                                         "token"]),
    "redmine":     ("/redmine/", ["key"]),
    "opencart":    ("/opencart/", ["route=checkout/cart", "route=checkout/checkout",
                                      "route=checkout/confirm", "route=checkout/success",
                                      "route=checkout/failure", "route=checkout/payment_address",
                                      "route=checkout/payment_method", "route=checkout/shipping_address",
                                      "route=checkout/shipping_method", "route=checkout/register",
                                      "route=account/account", "route=account/login",
                                      "route=account/logout", "route=account/register",
                                      "route=account/forgotten", "route=account/edit",
                                      "route=account/password", "route=account/address",
                                      "route=account/order", "route=account/wishlist",
                                      "route=account/download", "route=account/returns",
