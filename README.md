...
| `cache_turbo_store_head on\|off` | `server`, `location` | `off` | On a `HEAD` miss, issue one internal background **GET** so the GET response populates the cache; the HEAD response itself remains header-only and is marked so it is not stored as an incomplete body. |
...
| `POST /_cache?all=1` | Purge the whole zone (and the L2 keyspace, if Redis is on). L1 walks a finite snapshot-sized budget so concurrent refill cannot starve the worker; if entries remain after that budget, the response is `500` with `{"purged":N,"l1":"incomplete"}` (retry the purge). The L2 side is a `SCAN MATCH <prefix>*` walk; if that walk does **not** finish — read timeout, malformed reply, or the internal page cap — the response is `500` with `{"purged":N,"l2":"incomplete","reason":"…"}` and part of L2 still holds entries. If the walk cannot even start — L2 unreachable, connect refused — the response is `500` with `{"purged":N,"l2":"unavailable"}` and L2 is untouched. A `200` means L1 was empty at its final lock-held observation and the L2 walk reached the end of the keyspace. |
