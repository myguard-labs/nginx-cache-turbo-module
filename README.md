    ...
    server {
        ...
        location / {
          ...
          # cache_turbo_use_stale       http_404 http_429;  # which statuses count as "down". Omitted = the default, ANY 5xx.
            ...
            cache_turbo_copy_threshold    0;         # 0 = off (always pin); a body at/under this size is
                                                       # copied into r->pool instead of pinned in shm (C2)
...
| `cache_turbo_copy_threshold SIZE` | `server`, `location` | `0` (off) | A body at/under this size is `ngx_pnalloc`+`ngx_memcpy`'d into `r->pool` under the zone mutex already held and served with no shm reference at all, instead of pinned with a reference (`ngx_http_cache_turbo_blob_acquire`) that a pool cleanup drops once the response drains. `0` disables the copy path entirely (every body is pinned, byte-for-byte the pre-C2 behaviour) — it does **not** mean "copy nothing bigger than zero bytes". Above the threshold, behaviour is unchanged. Off by default: a `ci/tools/bench.sh` sweep (0/512/4k/16k/64k, tiny 272B + medium 270KB bodies, `PASSES=3`) could not resolve a throughput difference between the copy and pin path above this box's own run-to-run spread at either fixed size — a null result, not a measured win, so the directive ships opt-in rather than on by default. Worth trying on a workload with many small (sub-4k) hot bodies, where the saved atomic refcount + pool-cleanup registration may matter more than it did here. |
