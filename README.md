...
| `POST /_cache?url_file=<path>` | Same as `?url=`, but the list of paths comes from a file on disk (one `path[?query]` per line, CRLF tolerated) instead of the query string — useful when the list is longer than comfortably fits in a URL. Requires nginx built with `--with-threads` and an available/default `thread_pool`; the file read is posted through nginx's thread pool, and a missing prerequisite is a clean `500` with a JSON error body plus an error-log entry. Subject to the same `cache_turbo_warm_max` cap, plus its own bounded read: the file must be a regular file no larger than 64 KiB, and no single line may exceed 2048 bytes; either limit, or a missing/unreadable file, is a clean `500` with a JSON error body, never a crash or a silent partial warm. |
