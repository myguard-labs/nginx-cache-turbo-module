...
# (tools/bench.sh, tools/soak.sh). Not part of the Debian package build.
#
# wrk 4.1.0-4+b1 (Debian package) — HTTP load generator used by tools/bench.sh.
#   apt-get install wrk
#
# gawk 1:5.2.1-2 (Debian package) — tools/bench.sh PASSES>1 median/min/max
#   uses gawk's asort(); the default /usr/bin/awk (mawk) lacks it.
#   apt-get install gawk
#
# time 1.9-0.2 (Debian package) — provides /usr/bin/time -v, used by
#   tools/bench.sh to record wrk's own CPU% per pass (contention
